...
def check_cached_data_exists():
    ...
    # assert os.path.exists("./sniper-linear-classifier-symbol.json"), "demo/cache/indices.pkl was not found"
    # assert os.path.exists("./sniper-linear-classifier-params.json"), "demo/cache/indices.pkl was not found"
    assert os.path.exists(PARAM_FILE), PARAM_FILE + " was not found"
    assert os.path.exists(JSON_FILE), JSON_FILE + " was not found"
    # assert os.path.exists(MODEL_FILE), MODEL_FILE + " was not found"
...
def main():
    ...
    dir_names = [f for f in listdir("./portal/test/image/") if isdir(join("./portal/test/image", f))]
    image_names_full = []
    image_num_per_class = []
    ...
    # NOTE: Load Linear Classifier
    # linear_classifier = mx.mod.Module.load(MODEL_PREFIX, MODEL_EPOCH)
    # linear_classifier.bind(data_iter.provide_data, data_iter.provide_data, for_training=False)

    sym, arg_params, aux_params = mx.model.load_checkpoint(MODEL_PREFIX, MODEL_EPOCH)
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    # assert sym.tojson() == net.tojson()
    linear_classifier = mx.mod.Module(symbol=sym, data_names=['data'],
    ...
    # linear_classifier.save("sniper-linear-classifier")
    # logger = logging.getLogger()
    # logger.setLevel(logging.INFO)
    # linear_classifier_symbol = mx.symbol.load(MODEL_FILE)
    # linear_classifier = mx.mod.Module(symbol=linear_classifier_symbol, data_names=['data'], label_names=['softmax_label'],
    # 						logger=logger, context=mx.gpu())
    # linear_classifier.bind()
    # linear_classifier.load_params(PARAM_FILE)

    # with open("./demo/cache/data.pkl", 'rb') as p:
    #     linear_classifier = pickle.load(p)
    # linear_classifier.load_checkpoint(MODEL_PREFIX, MODEL_EPOCH)
    ...
    for test_idx in range(len(rois)):
        test_im_name = image_names_test[idx]
